function slider() {
  var filterData = [500000, 1000000, 1500000, 2000000, 2500000, 3000000, 3500000, 4000000, 4500000, 5000000];

  var rewardFreqChart = d3.select("#rewardFreqChart")

  rewardFreqChart.append("image")
    .attr("xlink:href", "./img/a2c_reward_freq/a2c_reward_freq_500000.png")
    .attr("width", 1500)
    .attr("height", 350);

  var filterSliderTimestep = d3
    .sliderBottom()
    .min(d3.min(filterData))
    .max(d3.max(filterData))
    .step(500000)
    .width(800)
    .tickFormat(d3.format(".3n"))
    .tickValues(filterData)
    .default(0.015)
    .on("onchange", val => {
      d3.selectAll("#rewardFreqChart > *").remove();
      d3.select("h3#filter-value-time").text("Timestep " + val);

      rewardFreqChart.append("image")
        .attr("xlink:href", "./img/a2c_reward_freq/a2c_reward_freq_" + val +".png")
        .attr("width", 1500)
        .attr("height", 350);
    });

  var filterSlider = d3
    .select("div#filter-slider-time")
    .append("svg")
    .attr("width", 900)
    .attr("height", 100)
    .append("g")
    .attr("transform", "translate(30,30)");
  filterSlider.call(filterSliderTimestep);
  d3.select("h3#filter-value-time").text("Timestep " + filterSliderTimestep.value());
}
slider();
...
function score() {
  // data for dqn scores
  var url = "https://z3n-t4n.github.io/IndividualProject/a2c_scores.json";
  ...
  var url3 = "https://z3n-t4n.github.io/IndividualProject/a2c_scores_filter.json";
  ...
  d3.json(url3).then( data => {
    ...
    var yScale = d3
      .scaleLinear()
      .range([height - spacing, 0])
      .domain([0, d3.max(data, function(d){ return d.mean; }) + 180]);
...
function reward() {
  ...
  var url = "https://z3n-t4n.github.io/IndividualProject/a2c_rewards.json";
...
function action() {
  ...
  var url = "https://z3n-t4n.github.io/IndividualProject/a2c_actions.json";
