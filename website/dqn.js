function score() {
  ...
  // data for agent of interest scores
  var url2 = "https://z3n-t4n.github.io/IndividualProject/dqn/agent_of_interest/scores.json";
  ...
  d3.json(url2).then( data => {
    ...
    // agent of interest
    d3.select("#aoiScores").on("change",aoiScores);
    aoiScores();

    function aoiScores() {
      var filter = scoreChart.selectAll("circle.aoi")
        .data(data);

      if(d3.select("#aoiScores").property("checked")){
        filter.enter()
          .append("circle")
          .attr("class", "aoi")
          .merge(filter)
          ...
          .on("click", function(event, d) {
            d3.select("p#scoreSelected")
              .text("Agent of Interest Selected: Timestep " + d.timestep);
            d3.select("h3#agentSelected")
              .text("Agent @ Timestep " + d.timestep);
            d3.select("#parallelCoordImg")
              .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/parallel_coord.png");
            d3.select("#rewardFreqImg")
              .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/reward_freq.png");
            d3.select("p#rewardSelected")
              .text("Agent of Interest Selected: N/A");
            if (d.timestep == 1300000) {
              d3.select("p#result")
                .text("Agent learns a new strategy");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/strategy.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/strategy.gif");
            }
            if (d.timestep == 4300000) {
              d3.select("p#result")
                .text("Agent fails to catch the ball");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/fail.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/fail.gif");
            }
          })
          .style("opacity", 1);
    	} else {
...
function reward() {
  ...
  // data for agent of interest rewards
  var url2 = "https://z3n-t4n.github.io/IndividualProject/dqn/agent_of_interest/rewards.json";
  ...
  d3.json(url2).then( data => {
    var xScale =  d3
      .scaleLinear()
      .range([0, width - spacing])
      .domain([0, d3.max(data, function(d){ return d.timestep; }) + 600000]);

    var yScale = d3
      .scaleLinear()
      .range([height - spacing, 0])
      .domain([0, d3.max(data, function(d){ return d.reward; }) + 690]);
    ...
    // points of interest
    d3.select("#aoiRewards").on("change",aoiRewards);
    aoiRewards();

    function aoiRewards() {
      var filter = rewardChart.selectAll("circle.aoi")
        .data(data);

      if(d3.select("#aoiRewards").property("checked")){
        filter.enter()
          .append("circle")
          .attr("class", "aoi")
          .merge(filter)
          ...
          .on("click", function(event, d) {
            ...
            d3.select("p#scoreSelected")
              .text("Agent of Interest Selected: N/A");
            if (d.timestep == 2250000) {
              d3.select("p#result")
                .text("Agent stuck in a loop")
              d3.select("#rewardFreqEp")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/loop.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/loop.gif");
            }
            if (d.timestep == 4400000) {
              d3.select("p#result")
                .text("Agent stuck in a loop");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/dqn/agent_of_interest/timestep_" + d.timestep + "/loop.png");
