function score() {
  ...
  var url2 = "https://z3n-t4n.github.io/IndividualProject/a2c/agent_of_interest/scores.json";
  ...
  d3.json(url2).then( data => {
    var xScale =  d3
      .scaleLinear()
      .range([0, width - spacing])
      .domain([0, d3.max(data, function(d){ return d.timestep; }) + 1800000]);
        ...
        filter.enter()
          ...
          .on("click", function(event, d) {
            ...
            d3.select("#parallelCoordImg")
              .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/parallel_coord.png");
            d3.select("#rewardFreqImg")
              .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/reward_freq.png");
            d3.select("p#rewardSelected")
              .text("Agent of Interest Selected: N/A");
            if (d.timestep == 1350000) {
              d3.select("p#result")
                .text("Agent learns a new strategy");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/strategy.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/strategy.gif");
            }
            if (d.timestep == 2700000) {
              d3.select("p#result")
                .text("Agent fails to catch the ball");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/fail.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/fail.gif");
            }
            if (d.timestep == 3200000) {
              d3.select("p#result")
                .text("Agent repeats strategy twice in a row");
              d3.select("#rewardFreqEp")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/repeat.png");
              d3.select("#rewardFreqGif")
                .attr("src", "./img/a2c/agent_of_interest/timestep_" + d.timestep + "/repeat.gif");
            }
          })
          .style("opacity", 1);
    	} else {
